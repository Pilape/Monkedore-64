...
void TestAddCarry(CuTest* tc) {
    // Carry set (carry becomes 0)
    {
        monkedore_Byte program[] = {
            0x02, 0x40, 0x00, 0x02, 0x20, 0x22, 0x0F, 0x01,
        };
        VM_INIT();
        ...
        CuAssertIntEquals(tc, 0x4000+0x2022+1, VM_STACK_TOP(vm.data_stack));
        CuAssertIntEquals(tc, 0, vm.carry);
    }

    {
        monkedore_Byte program[] = {
            0x02, 0xff, 0xff, 0x02, 0x23, 0x24, 0x0F, 0x01,
        };
        VM_INIT();
...
void TestSubCarry(CuTest* tc) {
    // Carry set (carry becomes 0)
    {
        monkedore_Byte program[] = {
            0x02, 0x40, 0x00, 0x02, 0x20, 0x22, 0x10, 0x01,
        };
        VM_INIT();
    ...
    // No carry set (carry becomes 1)
    {
        monkedore_Byte program[] = {
            0x02, 0x25, 0x24, 0x02, 0xFF, 0xFF, 0x10, 0x01,
        };
        VM_INIT();
...
CuSuite* ArithmeticGetSuite() {
    ...
    SUITE_ADD_TEST(suite, TestAddCarry);
    SUITE_ADD_TEST(suite, TestSubCarry);
    return suite;
}
